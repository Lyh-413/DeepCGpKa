...
## File Details
The folders oneb, twob, and oneb+ respectively contain one-bead model,two-bead model and one-bead model with additional angle features. These pKa shift values are ranging from -2 to 2. The twob-all folder contains two-bead model with all pKa shift values. In each folder, cphmd.py and exp67s.py are scripts for constructing input data modeling for the cphmd and exp67s datasets under the corresponding model representations. DCGPKA-exp67s.py and DCGPKA-CpHMD.py are scripts for pKa prediction on the corresponding datasets. 
